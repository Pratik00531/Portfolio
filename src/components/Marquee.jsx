...
const Marquee = ({ items,
  ...
  iconClassName = "text-gold size-4 md:size-6",
  reverse = false,
  ...
  useGSAP(() => {
    ...
    const initAnimation = () => {
      // Provide valid target for initialization if items are present
      if (!itemsRef.current.length || !itemsRef.current[0]) return;

      // Clean up previous instances to prevent overlaps on resize
      if (tl) tl.kill();
      if (observer) observer.kill();

      // Revert styles safely before recompiling loop
  ...
  }, { dependencies: [items, reverse], scope: containerRef });
